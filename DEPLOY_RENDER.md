# 🚀 Деплой на Render

## ⚡ Быстрый старт

### Способ 1: Автоматический деплой (рекомендуется)

#### Шаг 1: Загрузи код на GitHub

```bash
git add .
git commit -m "Filin Bot ready for Render"
git push origin main
```

#### Шаг 2: Подключи Render к GitHub

1. Зайди на [render.com](https://render.com)
2. Войди через GitHub
3. Нажми **"New +"** → **"Web Service"**
4. Выбери репозиторий `Filin`

#### Шаг 3: Настрой сервис

| Параметр | Значение |
|----------|----------|
| **Name** | `filin-webapp` |
| **Region** | Frankfurt (Germany) |
| **Branch** | `main` |
| **Root Directory** | (оставь пустым) |
| **Runtime** | `Python 3` |
| **Build Command** | `pip install -r requirements.txt` |
| **Start Command** | `uvicorn app.webapp.app:app --host 0.0.0.0 --port $PORT` |
| **Instance Type** | `Free` |

#### Шаг 4: Добавь переменные окружения

В Render Dashboard → Environment Variables добавь:

```
BOT_TOKEN=8306362120:AAHXCXOXFk_Eam6gbfnwK0f0vTyI16RNFZo
WEBAPP_URL=https://filin-webapp.onrender.com
ADMIN_IDS=1698158035
WORKERS_CHAT_ID=-1003748695791
DATABASE_URL=sqlite+aiosqlite:///./filin.db
LOG_PATH=logs.txt
DEFAULT_SCHEDULE=Ежедневно с 14:00 до 2:00
DEFAULT_CONTACTS=Phone: +7 (000) 000-00-00\nAddress: Example street, 1
```

#### Шаг 5: Deploy!

Нажми **"Create Web Service"** и жди ~2-3 минуты.

Render автоматически:
- ✅ Установит зависимости
- ✅ Запустит FastAPI сервер
- ✅ Даст HTTPS URL
- ✅ Настроит авто-деплой при `git push`

---

### Способ 2: Через render.yaml (автоматическая конфигурация)

1. Загрузи `render.yaml` в репозиторий
2. В Render нажми **"New +"** → **"Blueprint"**
3. Подключи репозиторий
4. Render автоматически применит настройки из `render.yaml`

---

## 🔧 Проверка работы WebApp

### 1. Проверь корневой URL

Открой в браузере:
```
https://filin-webapp.onrender.com
```

Должно вернуться:
```json
{"status": "ok", "service": "Filin Hookah Bot"}
```

### 2. Проверь WebApp

Открой:
```
https://filin-webapp.onrender.com/webapp
```

Должна загрузиться HTML-страница с меню.

### 3. Проверь статику

Открой:
```
https://filin-webapp.onrender.com/static/styles.css
```

Должен загрузиться CSS файл.

### 4. Проверь API

Открой:
```
https://filin-webapp.onrender.com/health
```

Должно вернуться:
```json
{"status": "ok"}
```

---

## 📱 Настройка Telegram бота

### 1. Обнови WEBAPP_URL

После деплоя Render выдаст URL вида:
```
https://filin-webapp.onrender.com
```

Обнови переменную `WEBAPP_URL` в Render Dashboard:
```
WEBAPP_URL=https://filin-webapp.onrender.com/webapp
```

### 2. Перезапусти сервис

В Render Dashboard → Manual Deploy → **"Deploy Latest Commit"**

Или сделай commit:
```bash
git commit --allow-empty -m "Trigger redeploy"
git push
```

### 3. Настрой Menu Button

1. Открой @BotFather
2. `/mybots` → FilinHookah_bot
3. **Menu Button** → отправь URL: `https://filin-webapp.onrender.com/webapp`

---

## 🔍 Мониторинг и логи

### Логи
Render Dashboard → **Logs**

### Метрики
Render Dashboard → **Metrics**

### Авто-деплой
При `git push` в ветку `main` Render автоматически пересоберёт сервис.

---

## ❌ Частые проблемы

| Проблема | Решение |
|----------|---------|
| **404 на /webapp** | Проверь Start Command: `uvicorn app.webapp.app:app --host 0.0.0.0 --port $PORT` |
| **Не грузится статика** | Проверь что файлы в `app/webapp/static/` |
| **500 ошибка** | Смотри логи в Render Dashboard |
| **Бот не отвечает** | Проверь BOT_TOKEN в переменных |
| **WebApp пустой** | Проверь консоль браузера (F12) на ошибки JS |
| **Спит сервис** | Free тариф засыпает через 15 мин без активности. Используй [UptimeRobot](https://uptimerobot.com) для пинга. |

---

## 💰 Тарифы Render

| Тариф | Цена | Особенности |
|-------|------|-------------|
| **Free** | $0 | Засыпает через 15 мин, 750 часов/мес |
| **Starter** | $7/мес | Не засыпает, 500 ГБ трафика |

**Для начала хватит Free тарифа!**

---

## 🎯 Итоговый чеклист

- [ ] Код загружен на GitHub
- [ ] Сервис создан на Render
- [ ] Переменные окружения добавлены
- [ ] `/health` возвращает `{"status": "ok"}`
- [ ] `/webapp` открывает HTML страницу
- [ ] `/static/styles.css` загружается
- [ ] Кнопка в боте открывает WebApp
- [ ] Menu Button настроен

**Готово! 🦉**
